...
def save_file_B(file_index,second_repository_name, file_format_B, file_size_limit_B):
    global is_header
...
def excel_file(dest_file_name):
    ...
    if is_header == True or is_first == True:
        ...
        sheet_1 = wb.active
        sheet_1.title = '시뮬레이션_서울특별시_관광지별_방문객'
        sheet_1.append(header_list)
        is_header = False
        is_first = False
    else:
        wb =openpyxl.load_workbook(filename=dest_file_name,read_only=False,data_only=False)
        sheet_1 =wb.active
    for index in range(simulation_count):
        sheet_1.append(simulation_data)
    wb.save(dest_file_name)

def save_file(index, repository_name, file_format, file_size_limit):
    dest_file_name = save_file_B(index, repository_name, file_format, file_size_limit)
    if file_format == 'xlsx':
        excel_file(dest_file_name)
    ...
    if choice == 1:
        ...
        if type_choice == 1:
            ...
            if not os.path.exists(initial_file_name_B):
                is_first = True
                save_file(1,Type_B_repository, file_format_B, file_size_limit_B)
            else:
                save_file(file_count_B(),Type_B_repository,file_format_B,file_size_limit_B)
        elif type_choice == 3:
            continue
        print("메인 기능 수행")
