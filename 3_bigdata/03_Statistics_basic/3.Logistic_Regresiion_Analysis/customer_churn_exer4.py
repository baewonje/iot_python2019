...
match_dic ={}
colums_list = ['account_length', 'area_code', 'intl_plan',
...
for num in range(1,len_colums+1):
    ...
    for tup in combi_list:
        colums_list_combi = list(tup)
        independent_variables = churn[colums_list_combi]
        ...
        for index in range(len(y_predicted_rounded)):
            if y_predicted_rounded[index] == dependent_variable.values[index]:
                match_count += 1
        print(f'\n독립변수 조합 >> {colums_list_combi}')
        print('>> match count = ', match_count)
        print('>> 정답률: %.2f%%'%(match_count / len(y_predicted_rounded)*100))
        match_dic[' '.join(colums_list_combi)] = match_count / len(y_predicted_rounded) *100

#접답률 최대값 찾기
match_dic = sorted(match_dic.items(), key=operator.itemgetter(1),reverse=True)
#print(match_dic)
print(f'\n\n 독립변수 최적화 분석 결과')
print(f'총 조합 갯수: %d'%len(match_dic))
