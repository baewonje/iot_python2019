# 목적: 독립변수 중 하나의 단위 변화에 대한 종족변수의 변화 평가
import numpy as np
import pandas as pd
...
cust_serv_mean = float(logit_model.params[0]) + \
                 float(logit_model.parmas[1]) * float(churn['account_length'].mean()) + \
...
print("Probaility of churn when account length changes by 1: %.2f"
