...
def whole_db():
    output_full = []
    mysql_set.execute("SELECT * FROM student_info b left outer join"
                      " student_languages s on b.student_id = s.student_id;")
    rows = mysql_set.fetchall()
    for row in rows:
        output = []
        for column_index in range(len(row)):
...
def insert_db():
    ...
    while True:
        ...
        count = 0
        mysql_set.execute("""INSERT INTO %s VALUES ('%s', '%s', '%s', '%s', '%s', '%s');"""
                          % (table_list, ID, name_value, sex_value, age_value, major_value))
        for name in language_level:
...
def whole_search_db(num_data):
    ...
    mysql_set.execute("select * from student_info b left outer join student_languages"
                      "s using(student_id) where %s like '%%%s%%';" %(mode, search_data))
...
def update_db():
    ...
    try:
        mysql_set.execute("SELECT * FROM student_info b left outer join"
                          " student_languages s on b.student_id = s.student_id where b.student_id='%s';" % update_id)
    except Exception:
        pass
    ...
    if output_full[0][6] != 'None':
        ...
        for language_value in output_full:
            ...
            num += 1
            print("%d.전공: %s" % (num, language_value[8]))
            num += 1
    ...
    if update_info == '1':
        mysql_set.execute("update %s set name='%s' where student_id='%s';"
                          % (table_list, update_data,update_id))
    elif update_info == '2':
        mysql_set.execute("update %s set sex='%s' where student_id='%s';"
                          % (table_list,update_data,update_id))
    elif update_info == '3':
        mysql_set.execute("update %s set age='%s' where student_id='%s';"
                          % (table_list,update_data,update_id))
    elif update_info == '4':
        mysql_set.execute("update %s set major='%s' where student_id='%s';"
                          % (table_list,update_data,update_id))
    if output_full[0][6] != 'None':
        count += 3
        count2 = 0
        ...
        for language_value in output_full:
            if (int(update_info) <= count + 3 and int(update_info) > count):
                if int(update_info) % 3 == 2:
                    mysql_set.execute("update %s set name='%s' where student_id='%s' and name='%s';"
                                      %(table_language, update_data, update_id,language_value[6]))
                if int(update_info) % 3 == 0:
                    mysql_set.execute("update %s set level='%s' where student_id='%s' and name='%s';"
                                      %(table_language, update_data, update_id,language_value[6]))
                if int(update_info) % 3 == 1:
                    mysql_set.execute("update %s set period='%s' where student_id='%s' and name='%s';"
                                      %(table_language, update_data, update_id,language_value[6]))
    ...
    mysql_set.execute("SELECT * FROM student_info b left outer join student_languages s on"
                      "b.student_id = s.student_id where b.student_id='%s';" % update_id)
...
def insert_csv():
    try:
        ...
        mysql_set.execute("CREATE TABLE IF NOT EXISTS student_info"
                          "(Student_ID VARCHAR(20), Name VARCHAR(20),"
                          " sex VARCHAR(20), Age int, Major VARCHAR(20));")
        con.commit()
    try:
        mysql_set.execute('desc %s'%table_language)
    except Exception:
        mysql_set.execute("CREATE TABLE IF NOT EXISTS student_languages"
                          "(Student_ID VARCHAR(20), Name VARCHAR(20),"
                          " level VARCHAR(20), period VARCHAR(20));")
        con.commit()
