...
# 데이터 출처
# https://archive.ics.uci.edu/ml/datasets/Online_Retail
...
# 데이터 구조 정의
# 사용자 ID를 키로, 상품코드의 셋을 밸류로 갖는 딕셔너리
user_product_dic = {}
# 상품 코드를 키로, 사용자의 ID의 셋을 밸류로 갖는 딕셔너리
product_user_dic = {}
...
def analyze_clusters_keywords(labels, product_id_name_dic, user_product_dic, id_user_dic):
    ...
    for cluster_id, product_dame in cluster_item.items():
        # 각 클러스터안의 상품명을 join명령으로 합쳐 하나의 문자열로 만든뒤
        # 스페이스 혹은 탭으로 스플릿하여 키워드로 분해
        product_dame_keyword = (" ").join(product_dame).split()

        # 클러스터의 아이디와, 그 아이디를 가지는 클러스터에 속하는 유저들이
        # 구매한 상품들의 상품명안에
        # 가장 자주 나타나는 단어 20개를 역순으로 출력
        print("cluster_id:", cluster_id)
        print(Counter(product_dame_keyword).most_common(20))
...
for line in open('online_retail_utf.txt'):
    ...
    # {'22386': 'JUMBO BAG PINK POLKADOT'} <== product id 별 상품 이름
...
# skewness=5.833816723404396,kurtosis=72.600364097540136)
# nobs: # of observations
# skewness: 값의 쏠림 정도, 양의수(positive): 정규분포에서 왼쪽으로 쏠리는 경우,
# 음의수(negative): 오른쪽으로 쏠림
# kurtosis: 정규분포상에 값의 쏠림을 나타내는 다른 지표, 값이 클 수록 쏠림 현상이 강하다.
# 구매한 상품의 가짓수가 1인 사용자의 사용자의 사용자 ID를 검색
...
# 찾아낸 사용자를 군집화에 사용할 user_product_dic애서 제외
...
for k in range(2,9):
    ...
    # 실루엣 게수: 생성된 클러스터 안의 데이터가 얼마나 밀접하게 모여 있는지를 계산
...
# example] cluster0: RED RETROSPOT 슽일을 선호하고 구매스타일이 상대적으로 단조롭다.
#        => 선호 제품 우선순으로 기획 상품 마케팅, 선호제품 다변화
#  example] cluster1: RED RETROSPOT 스타일을 선호하지만 구매스타일이 매우 다양하다.
#       => 신규 상품 중 RED RETROSPOT 스타일 출시시 자동 추천
